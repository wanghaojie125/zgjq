  ...
  Formatter.prototype.beautify = function($contents) {
    ...
    return $contents.each((function(_this) {
      return function(i, el) {
        var $el;
        $el = $(el);
        if ($el.is(':not(images, br, col, td, hr, [class^="simditor-"]):empty')) {
          $el.remove();
        }
        if (uselessP($el)) {
          $el.remove();
        }
        return $el.find(':not(images, br, col, td, hr, [class^="simditor-"]):empty').remove();
      };
    })(this));
  };
          ...
          if (pasteContent.is('p')) {
            ...
            if (children.length === 1 && children.is('img')) {
              ...
              if (/^data:image/.test($img.attr('src'))) {
                ...
                return;
              } else if ($img.is('images[src^="webkit-fake-url://"]')) {
                return;
              }
            }
...
ImageButton = (function(_super) {
  ...
  ImageButton.prototype._init = function() {
    ...
    this.editor.body.on('click', 'images:not([data-non-image])', (function(_this) {
      return function(e) {
        var $img, range;
        $img = $(e.currentTarget);
        range = document.createRange();
    ...
    this.editor.body.on('mouseup', 'images:not([data-non-image])', (function(_this) {
      return function(e) {
        return false;
      };
    })(this));
    this.editor.on('selectionchanged.image', (function(_this) {
      return function() {
        ...
        if ($contents.length === 1 && $contents.is('images:not([data-non-image])')) {
